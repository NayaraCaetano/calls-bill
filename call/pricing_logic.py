...
def calc_call_cust(start, end):
    ...
    for call in calls_day:
        for rule in RULES:
            duration_minutes = _get_rule_duration(rule['start'], rule['end'], call['start'], call['end'])
...
def _get_rule_duration(rule_start, rule_end, start, end):
    date_rule_start = pytz.utc.localize(datetime.combine(start.date(), rule_start))
    date_rule_end = pytz.utc.localize(datetime.combine(start.date(), rule_end))
